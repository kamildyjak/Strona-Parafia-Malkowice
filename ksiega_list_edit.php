...
function remove(get_cel){
	...
	switch (cel){
		<?php
			$sql_ksiega_list = "SELECT id,data,autor,tresc FROM ksiega_gosci WHERE ok='tak' ORDER BY id DESC";
			$wynik_ksiega_list = $conn->query($sql_ksiega_list);
			while($row_ksiega_list=$wynik_ksiega_list->fetch_assoc()){
				$append = '<center><h1>Potwierdź wykonanie operacji ! </h1><h2>Czy na pewno chcesz <u><b>usunąć</b></u> wpis autora :</h2><h3>'. $row_ksiega_list['autor']. ' ?</h3><article><a href="delete_ksiega.php?id='. $row_ksiega_list['id'].'">TAK</a></article><article><a href="javascript:delete_no()">NIE</a></article></center>';
...
<div id="header_list">
<center><p>Lista wpisów w księdze gości</p></center>
</div>
...
<?php 
$sql_ksiega_li = "SELECT id,data,autor,tresc FROM ksiega_gosci WHERE ok='tak' ORDER BY id DESC";
	$wynik_ksiega_li = $conn->query($sql_ksiega_li);
	while($row_ksiega_li=$wynik_ksiega_li->fetch_assoc()){
		$ksiega_id = $row_ksiega_li['id'];
		echo "<li><p>Autor : <b>".$row_ksiega_li['autor']."</b></p><p>". $row_ksiega_li['temat']. "</p><p style='margin-left:15px'>/".$row_ksiega_li['data']."/</p><a href='javascript:remove(". $ksiega_id. ");'><img src='images/trash_icon_19.png'></a></li>";
	}
?>
